...
1. OS（macOS / Linux）を自動判定
2. 不足している依存（git / ffmpeg / uv）を **Homebrew経由で自動インストール**
3. プロジェクトを `~/nhk_gogaku_2026` に配置
...
`bootstrap.sh` は **依存ツールのチェックのみ** を行い、不足があれば **OSに応じたインストールコマンドを表示して終了** します（勝手に `sudo` や `brew install` は実行しません）。

必要なツール:
- **git** / **curl** / **python3** / **ffmpeg** ... 必須
- **uv** ... `manage_series.py` (CLI で番組選択) を使う場合のみ。Web UI からの操作なら不要

不足が出たら、表示されたコマンドをコピペで実行してください。例:
